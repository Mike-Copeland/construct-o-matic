...
A Python tool that builds a **digital twin** from construction plan set PDFs. It uses the Google Gemini API to extract features from every sheet type (plan & profile, drainage, signing, pavement markings, erosion control, traffic control) and converts them into geo-referenced Points, LineStrings, and Polygons — viewable in Google Earth or any GIS platform.
...
*   Used to assign default offsets when a feature's offset isn't explicitly specified in the plans.
...
*   Routes each feature to the appropriate geometry builder:
    ...
    *   **Polygons:** start/end station + width band (crosswalks, pavement areas)
*   All geometry passes through the same proven COGO pipeline: station → interpolate along centerline → perpendicular offset → grid-to-WGS84 transform.
